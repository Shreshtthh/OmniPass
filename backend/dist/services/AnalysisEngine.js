...
class AnalysisEngine {
    constructor(isMainnet = false) {
        ...
        this.alchemyService = new AlchemyService_1.AlchemyService();
    ...
    async performCrossChainAnalysis(address) {
        ...
        try {
            ...
            const chains = await this.buildChainData(address, ethBalance, polygonBalance, ethTvl, polygonTvl, ethTokens, polygonTokens);
    ...
    async calculateChainTVL(nativeBalance, tokenBalances, nativePrice) {
    ...
    async buildChainData(address, ethBalance, polygonBalance, ethTvl, polygonTvl, ethTokens, polygonTokens) {
        const chains = [
            {
    ...
    buildProtocolData(chainName, nativeBalance, tokens, totalTvl) {
        const protocols = [];
    ...
    calculateEnhancedRiskScore(chains, portfolioMetrics, walletAge) {
        let score = 0;
        let maxScore = 0;
        ...
        chains.forEach(chain => {
            chain.protocols.forEach(protocol => {
                if (protocol.healthFactor && protocol.healthFactor !== 1.0) {
                    if (protocol.healthFactor >= this.healthFactorRating.excellent)
                        healthScore += 30;
                    else if (protocol.healthFactor >= this.healthFactorRating.good)
                        healthScore += 25;
                    else if (protocol.healthFactor >= this.healthFactorRating.moderate)
                        healthScore += 15;
                    else
                        healthScore += 5;
                    healthCount++;
    ...
    calculateEnhancedActivityScore(chains, portfolioMetrics) {
        let score = 0;
        ...
        if (portfolioMetrics.totalProtocols >= 3)
            score += 25;
        else if (portfolioMetrics.totalProtocols >= 2)
            score += 15;
        else if (portfolioMetrics.totalProtocols >= 1)
            score += 10;
        // Position complexity (20 points)
        const totalPositions = chains.reduce((sum, chain) => sum + chain.protocols.reduce((pSum, protocol) => pSum + protocol.positions, 0), 0);
        if (totalPositions >= 5)
            score += 20;
        ...
        if (portfolioMetrics.totalTvl >= 5000)
            score += 15;
        else if (portfolioMetrics.totalTvl >= 1000)
            score += 12;
        else if (portfolioMetrics.totalTvl >= 500)
            score += 8;
        else if (portfolioMetrics.totalTvl >= 100)
            score += 5;
        // DeFi protocol usage bonus (10 points)
        const hasDeFi = chains.some(chain => chain.protocols.some(protocol => protocol.name !== 'Wallet Balance' && protocol.healthFactor && protocol.healthFactor !== 1.0));
        if (hasDeFi)
            score += 10;
        return Math.min(score, 100);
    }
    calculateEnhancedDiversificationScore(chains, portfolioMetrics) {
        let score = 0;
        ...
        chains.forEach(chain => {
            chain.protocols.forEach(protocol => {
                if (protocol.name === 'Wallet Balance')
                    uniqueProtocolTypes.add('Wallet');
                else if (protocol.name.includes('Aave'))
                    uniqueProtocolTypes.add('Lending');
                else if (protocol.name.includes('Uniswap'))
                    uniqueProtocolTypes.add('DEX');
                else
                    uniqueProtocolTypes.add('Other');
            });
        });
        if (uniqueProtocolTypes.size >= 3)
            score += 35;
        else if (uniqueProtocolTypes.size >= 2)
            score += 25;
        else if (uniqueProtocolTypes.size >= 1)
            score += 15;
        return Math.min(score, 100);
    }
    calculatePortfolioMetrics(chains, totalTvl) {
    ...
    calculateChainConcentration(chains, totalTvl) {
        if (totalTvl === 0)
            return 0;
        const maxChainTvl = Math.max(...chains.map(c => c.tvl));
        return maxChainTvl / totalTvl;
    }
    calculateProtocolDiversity(chains) {
    ...
    assessRiskProfile(chains) {
        ...
        if (hasHighRisk)
            return 'High Risk';
        if (totalTvl > 0 && riskWeightedScore / totalTvl > 0.8)
            return 'Conservative';
        if (totalTvl > 0 && riskWeightedScore / totalTvl > 0.6)
            return 'Moderate';
        return 'Aggressive';
    }
    calculateAverageHealthFactor(chains) {
    ...
    calculateOverallScore(riskScore, activityScore, diversificationScore) {
        return Math.round((riskScore * 0.4) +
            (activityScore * 0.35) +
