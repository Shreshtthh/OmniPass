...
router.post('/issue-credential', async (req, res) => {
    ...
    try {
        ...
        const crossChainData = analysis.chains.map(chain => ({
            ...
            dataHash: ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(JSON.stringify(chain)))
        }));
        // Create AI insights hash
        const aiInsightsHash = ethers_1.ethers.keccak256(ethers_1.ethers.toUtf8Bytes(JSON.stringify(analysis.aiInsights)));
        // ✅ CALL YOUR CONTRACT SERVICE
        const txHash = await contractService.issueCredential(address, crossChainData, aiInsightsHash);
...
router.get('/chains', (req, res) => {
    const supportedChains = AlchemyService_1.AlchemyService.getSupportedChains();
    res.json({
        success: true,
