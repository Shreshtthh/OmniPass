...
export class AICoachService {
  ...
  private responseCache: Map<string, { response: CoachResponse; timestamp: number }> = new Map();
  ...
  private getCachedResponse(cacheKey: string): CoachResponse | null {
  ...
  private setCachedResponse(cacheKey: string, response: CoachResponse): void {
  ...
  getAvailableQuestions(userAnalysis?: CrossChainAnalysis): CoachQuestion[] {
    const baseQuestions: CoachQuestion[] = [
  ...
  private filterQuestionsForUser(questions: CoachQuestion[], analysis: CrossChainAnalysis): CoachQuestion[] {
  ...
  async answerQuestion(
    ...
    userAnalysis?: CrossChainAnalysis
...
- If user has low TVL, focus on educational testnet strategies first`;
  ...
  private parseCoachResponse(aiText: string, questionId: string, customQuestion?: string): CoachResponse {
    try {
  ...
  private getFallbackAnswer(questionId: string, customQuestion?: string): CoachResponse {
    const question = customQuestion || this.getQuestionText(questionId);
    
    const fallbackAnswers: Record<string, CoachResponse> = {
      'next-tier': {
  ...
  getSuggestedQuestions(userAnalysis: CrossChainAnalysis): string[] {
