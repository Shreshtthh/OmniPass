...
export class AlchemyService {
  ...
  private initializeConnections() {
    if (this.networkConfig.isMainnet) {
      ...
      this.providers = {
        ethereum: new ethers.JsonRpcProvider(this.baseUrls.ethereum),
        polygon: new ethers.JsonRpcProvider(this.baseUrls.polygon)
      };
    } else {
      this.baseUrls = {
  ...
  updateNetworkConfig(networkConfig: NetworkConfig): void {
  ...
  async getMainnetBalance(address: string): Promise<number> {
    try {
      if (!this.apiKey) {
  ...
  async getPolygonBalance(address: string): Promise<number> {
    if (this.networkConfig.isMainnet) {
  ...
  async getPolygonMainnetBalance(address: string): Promise<number> {
    try {
      if (!this.apiKey) {
  ...
  async getFirstTransaction(address: string): Promise<{ timestamp: number } | null> {
    try {
      if (!this.apiKey) {
